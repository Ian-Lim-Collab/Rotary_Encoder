...
:encoder_A(encoder_A_pin), encoder_B(encoder_B_pin), button(button_pin)
{
    ...
    button.fall(callback(this, &Rotory_Encoder::button_fall_callback));
...
void Rotory_Encoder::encoder_A_ISR_callback(){
    ...
    if(this->rotor_debounce_period.elapsed_time() > ROTORY_ENCODER_ROTORY_DEBOUNCE_PERIOD){
...
void Rotory_Encoder::button_rise_callback(){
    ...
    if(this->button_debounce_period.elapsed_time() > ROTORY_ENCODER_BUTTON_DEBOUNCE_PERIOD){
        btn_state = !btn_state;
    }
};

void Rotory_Encoder::button_fall_callback(){
